...
```
- Targeting the 'table' element above makes all tables default to a 1px black border.
- Targeting the ```#primary-table``` ID makes the element with ID of ```primary-table``` have a 3px red border.
- When rules conflict, *Targeting ID's takes precedence over targeting elements*
