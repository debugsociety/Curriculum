...
A design is *responsive* when it adapts to changes in the size of the device it is being viewed on. Rather than creating multiple versions of a website for different devices, such as iPhone, iPad, and Desktop, it is regularly more maintainable to create a single, responsive, website design.

Sometimes the CSS for diferent sized displays must differ. To accomplish this without creating two versions of the website, we utilize media queries.

A ```media query``` limits what a section of the CSS applies to, based on a user-defined expression, or query.

The 'expression' or 'query' can specify any of these device properties:
- The minimum size
- The maximum size
- The type (e.g. all, screen, print)
- The orientation (landscape or portrait)

I believe this is easiest explained through an example:
```
/* CSS that applies to all devices goes here */

@media (min-width: 700px) {
/* CSS that only applies to displays larger than 700px */
}

@media print and (orientation: landscape) and (min-width: 1000px) {
/* CSS that only applies to PRINT displays, in LANDSCAPE mode, larger than 1000px */
}
```
